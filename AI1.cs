...
public class AI1 : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (flashing)
        {
            GetComponent<SpriteRenderer>().color = new Color(0.818f, Mathf.Sin(Time.time * 90)*.75f + .25f, Mathf.Sin(Time.time * 90)*.75f + .25f);
        }
          
        //Updates based on score
        ...
        if (currentHealth <= 0)
        {
            ...
            if (player.GetComponent<UIController>().score >= 100 * (gameManager.numberOfKeys + 1))
            {
                ...
                player.GetComponent<UIController>().score += 1;
                Destroy(gameObject);
            }
